...
11. **`AH-011` Project-Specific Design Skill Routing [CRITICAL]**: Before implementing any Frontend UI component, the AI Agent MUST inspect `<design_data>` and `<taste_skill>` inside `.piardify/context.md` (or `design.md`) and automatically activate the corresponding Taste Skill from `<system_directives>`:
...
1. **Level 1 (Highest Priority - Ground Truth)**: **`design.md`** (or `<design_data>` in `.piardify/context.md`).
